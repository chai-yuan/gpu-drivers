...
static int bochs_hw_init(struct bochs_device *bochs) {
    ...
    mem = bochs_dispi_read(bochs, VBE_DISPI_INDEX_VIDEO_MEMORY_64K) * 64 * 1024;
...
static void bochs_hw_setmode(struct bochs_device *bochs, struct drm_display_mode *mode) {
    ...
    DRM_DEBUG_DRIVER("%dx%d @ %d bpp, vy %d\n", bochs->xres, bochs->yres, bochs->bpp, bochs->yres_virtual);
    ...
    bochs_dispi_write(bochs, VBE_DISPI_INDEX_VIRT_HEIGHT, bochs->yres_virtual);
    bochs_dispi_write(bochs, VBE_DISPI_INDEX_X_OFFSET, 0);
    bochs_dispi_write(bochs, VBE_DISPI_INDEX_Y_OFFSET, 0);

    bochs_dispi_write(bochs, VBE_DISPI_INDEX_ENABLE, VBE_DISPI_ENABLED | VBE_DISPI_LFB_ENABLED);
...
static void bochs_hw_setformat(struct bochs_device *bochs, const struct drm_format_info *format) {
    ...
    DRM_DEBUG_DRIVER("format %c%c%c%c\n", (format->format >> 0) & 0xff, (format->format >> 8) & 0xff,
                     (format->format >> 16) & 0xff, (format->format >> 24) & 0xff);
...
static void bochs_hw_setbase(struct bochs_device *bochs, int x, int y, int stride, u64 addr) {
    ...
    bochs->stride = stride;
    offset        = (unsigned long)addr + y * bochs->stride + x * (bochs->bpp / 8);
...
static int bochs_primary_plane_helper_atomic_check(struct drm_plane *plane, struct drm_atomic_state *state) {
...
static void bochs_primary_plane_helper_atomic_update(struct drm_plane *plane, struct drm_atomic_state *state) {
    ...
    struct drm_rect top_left_quarter = {
        .x1 = 0,
        .y1 = 0,
        .x2 = plane_state->crtc_w / 2, // 宽度的一半
        .y2 = plane_state->crtc_h / 2, // 高度的一半
    };

    drm_atomic_helper_damage_iter_init(&iter, old_plane_state, plane_state);
    drm_atomic_for_each_plane_damage(&iter, &damage) {
        if (!drm_rect_intersect(&damage, &top_left_quarter)) {
            continue; // 如果更新区域完全在左上角之外，直接跳过，不复制数据
        }
    ...
    bochs_hw_setbase(bochs, plane_state->crtc_x, plane_state->crtc_y, fb->pitches[0], 0);
...
static int bochs_crtc_helper_atomic_check(struct drm_crtc *crtc, struct drm_atomic_state *state) {
...
static void bochs_crtc_helper_atomic_enable(struct drm_crtc *crtc, struct drm_atomic_state *state) {
    struct bochs_device *bochs = to_bochs_device(crtc->dev);

    bochs_hw_blank(bochs, false);
}

static void bochs_crtc_helper_atomic_disable(struct drm_crtc *crtc, struct drm_atomic_state *crtc_state) {
    struct bochs_device *bochs = to_bochs_device(crtc->dev);
...
static enum drm_mode_status bochs_mode_config_mode_valid(struct drm_device *dev, const struct drm_display_mode *mode) {
    struct bochs_device          *bochs  = to_bochs_device(dev);
...
static int bochs_kms_init(struct bochs_device *bochs) {
    ...
    ret = drm_universal_plane_init(dev, primary_plane, 0, &bochs_primary_plane_funcs, bochs_primary_plane_formats,
                                   ARRAY_SIZE(bochs_primary_plane_formats), NULL, DRM_PLANE_TYPE_PRIMARY, NULL);
    if (ret)
        return ret;
    ...
    crtc = &bochs->crtc;
    ret  = drm_crtc_init_with_planes(dev, crtc, primary_plane, NULL, &bochs_crtc_funcs, NULL);
    if (ret)
        return ret;
    drm_crtc_helper_add(crtc, &bochs_crtc_helper_funcs);

    encoder = &bochs->encoder;
    ret     = drm_encoder_init(dev, encoder, &bochs_encoder_funcs, DRM_MODE_ENCODER_VIRTUAL, NULL);
    if (ret)
        return ret;
    encoder->possible_crtcs = drm_crtc_mask(crtc);

    connector = &bochs->connector;
    ret       = drm_connector_init(dev, connector, &bochs_connector_funcs, DRM_MODE_CONNECTOR_VIRTUAL);
    if (ret)
        return ret;
...
static const struct dev_pm_ops bochs_pm_ops = {0};
...
static void bochs_pci_shutdown(struct pci_dev *pdev) { drm_atomic_helper_shutdown(pci_get_drvdata(pdev)); }
